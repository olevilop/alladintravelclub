  ...
  return (
    <section id="hero" className="relative h-screen overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col items-start justify-center px-6 sm:px-12 lg:px-24">
        ...
        <div className="absolute bottom-12 left-6 sm:left-12 lg:left-24 right-6 sm:right-12 lg:right-24 flex items-center gap-6">
          <motion.button
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 1.2 }}
            onClick={() => document.querySelector("#tours")?.scrollIntoView({ behavior: "smooth" })}
            className="flex items-center gap-3 px-6 py-3 rounded-full text-white font-sans text-lg tracking-wide hover:brightness-110 transition-all duration-300 shrink-0"
